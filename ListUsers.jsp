		...
		<c:forEach items="${requestScope.ulist}" var="user" varStatus="a">
			<c:if test="${a.count mod 2 eq 0}">
				<tr bgcolor="#8470ff">
					...
					<td>${user.country}</td>
					<td>${user.gender}</td>
					<td><a href="/MVCproject/EditUserServlet?id=${user.userId}&action=edit">Edit</a></td>
                   <td><a href="/MVCproject/EditUserServlet?id=${user.userId}&action=delete">Delete</a></td>
				</tr>
			</c:if>
